...
void PathBellman(int H2[7],int vcurent, int indice)
{
	...
	if (vcurent == varf-1) 
	{
        	...
        	printf("%d ", curent[i]+1);
		    printf("\n");
    		vprec = k; //salveaza pozitia in drumul curent
    		for (int i = 0; i < k; i++) 
		{
        		prec[i] = curent[i];
    		}
	...
	for(int i = vcurent; i <= varf-1; i++)
	{
		if(matrice[vcurent][i] > 0)
		{
			if(H2[vcurent] - H2[i] == matrice[vcurent][i])
			{
				...
				indice = vcurent;//cand ajunge la capatul unui drum se intoarce in acest punct si reia procesul de la valoarea data
			}
		}
	}
}
