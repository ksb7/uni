public class StableState 
{
    ...
    public StableState(int processes, int resources) 
    {
        max = new int[processes][resources];
        allocated = new int[processes][resources];
        available = new int[resources];

        // Valori care duc la o stare sigura
        for (int i = 0; i < processes; i++) 
        {
            for (int j = 0; j < resources; j++) 
            {
                max[i][j] = 3;            // maxim necesar
                allocated[i][j] = 1;      // deja alocat puțin
            }
        }
        for (int j = 0; j < resources; j++) 
        {
            available[j] = 3;             // destule resurse libere
        }
    }
    ...
    {
        Synchronization sync = new Synchronization(3);
        for (int i = 1; i <= 8; i++) 
        {
            new ProcessThread(i, 200 + i * 100, sync).start();
        }
    }
}
